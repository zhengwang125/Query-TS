...
if __name__ == '__main__':
    ...
    for ratio in [0.0025]:
        ...
        RES = F.range_query_operator(DB_TREE, simDB_TREE, query1+query2, verbose=False)
        print('range query effectiveness (data distribution)', RES)
    ...
    for ratio in [0.0025]:
        ...
        simDB_TREE, _  = F.build_Rtree(sim_DB)
        RES = F.range_query_operator(DB_TREE, simDB_TREE, query1+query2, verbose=False)
        print('range query effectiveness (gau distribution)', RES)
